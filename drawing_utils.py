...
def plot_landmarks(landmark_list: landmark_pb2.NormalizedLandmarkList,
  ...
  # plt.savefig('./output_3d/3d_'+str(count_frame)+'.png')
  # count_png_num+=1
  # # fig = plt.figure()
  # fig, ax = plt.subplots()
...
def ori_plot_landmarks(landmark_list: landmark_pb2.NormalizedLandmarkList,
                   connections: Optional[List[Tuple[int, int]]] = None,
                   landmark_drawing_spec: DrawingSpec = DrawingSpec(
                       color=RED_COLOR, thickness=5),
                   connection_drawing_spec: DrawingSpec = DrawingSpec(
  ...
  for angle in range(0, 360,5):
    ...
    plt.draw()
    plt.pause(.001)
    plt.savefig('./anime_3d/'+str(index)+'.jpg')
    index+=1
    # plt.show()
